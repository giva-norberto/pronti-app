 ...
 *
 * Alterações / objetivos principais:
 * - TRIAL agora considera SOMENTE company.trialEndDate.
 * - Mantidas validações de pagamento/aprovação manual.
 * - Evita query "in" com mais de 10 ids (chunking quando necessário).
 * - Logs de DEBUG controlados por DEBUG_LOGS.
 */
...
onAuthStateChanged(auth, (user) => {
  if (user) {
    ...
    // Nota: caso outra parte do app dependa desta presença imediata, avaliar comportamento.
    localStorage.removeItem('empresaAtivaId');

    // Salva o primeiro nome do usuário para uso em outras telas
...
// Regras:
// - Mantém checagens de pagamento/aprovação manual.
...
async function carregarEmpresas(userId, nomeUsuario) {
  try {
    ...
    // Para múltiplas empresas, busca documentos em chunks para evitar limite do 'in' (10)
    const empresas = [];
    const CHUNK_SIZE = 10;
