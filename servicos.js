...
// servicos.js (VERSÃO FINAL ALINHADA COM O FIREBASE E COM SUPER-DEBUG)
...
async function excluirServico(servicoIdParaExcluir) {
    ...
    const confirmado = await showCustomConfirm("Confirmar Exclusão", "Tem certeza que deseja excluir este serviço? Esta ação é permanente.");
    if (!confirmado) {
        console.log("Ação de exclusão cancelada pelo usuário no modal.");
        return;
    }
...
// Adiciona um único "escutador" para a lista inteira, com SUPER DEBUG.
listaServicosDiv.addEventListener('click', function(e) {
    console.log("---------------------------------");
    console.log("PASSO 1: Clique detectado na lista de serviços!");

    // Usamos .closest() para garantir que pegamos o botão, mesmo que o clique seja num ícone dentro dele
    const target = e.target.closest('.btn-acao');

    if (!target) {
        console.log("FALHA PASSO 2: O clique não foi num botão com a classe '.btn-acao'. A sair.");
        console.log("---------------------------------");
        return;
    }

    console.log("PASSO 2: Botão de ação encontrado:", target);

    const id = target.dataset.id;
    if (!id) {
        console.log("FALHA PASSO 3: O botão não tem um atributo 'data-id'. A sair.");
        console.log("---------------------------------");
        return;
    }

    console.log("PASSO 3: ID do serviço encontrado:", id);

    if (target.classList.contains('btn-editar')) {
        console.log("PASSO 4: É um botão de EDITAR. A redireccionar...");
        window.location.href = `novo-servico.html?id=${id}`;
    }

    if (target.classList.contains('btn-excluir')) {
        console.log("PASSO 4: É um botão de EXCLUIR. A chamar a função 'excluirServico'...");
        excluirServico(id);
    }
    console.log("---------------------------------");
});
