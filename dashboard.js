// NOVO: Importe as funções de verificação e o alerta
import { checkUserStatus, ensureTrialStart } from "./userService.js";
...
// --- FUNÇÕES DE LÓGICA E DADOS (sem alterações) ---
...
        if (user) {
            // NOVO: Portão de verificação de acesso
            const status = await checkUserStatus();

            if (status.hasActivePlan || status.isTrialActive) {
                // ACESSO PERMITIDO: Executa a lógica original do dashboard
                iniciarDashboard(user);
            } else {
                // ACESSO NEGADO: Mostra o alerta para iniciar o trial
                const handleActivateTrial = async () => {
                    await ensureTrialStart();
                    alert("Período de teste ativado! A página será recarregada.");
                    window.location.reload();
                };

                const handleCancel = () => {
                    alert("Você precisa de um plano para continuar. Redirecionando para o login.");
                    window.location.href = "login.html";
                };

                showCustomAlert({
                    title: "Nenhum plano ativo",
                    message: "Para acessar seus dados, inicie seu período de teste gratuito de 15 dias.",
...
// NOVO: Função para encapsular a lógica original do dashboard
async function iniciarDashboard(user) {
