...
async function carregarDashboard(uid) {
  try {
    ...
    // Profissional: documento dentro de "empresarios/{empresaId}/profissionais/{uid}"
    ...
    if (profissionalSnapshot.exists() && profissionalSnapshot.data().servicos) {
      profissionalSnapshot.data().servicos.forEach(servico => {
...
// Processa e exibe o resumo diário com IA
...
// -------------------------
// GRÁFICOS (sem alterações essenciais)
...
function gerarGraficoServicos(servicosMap, agendamentos) {
  ...
  agendamentos.forEach(ag => {
    const servicoId = String(ag.servicoId);
