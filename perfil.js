 ...
 * Este script é controlado e inicializado pelo 'firebase-config.js'
 */

// As importações diretas do Firebase foram removidas daqui.
// Este arquivo agora recebe os serviços (auth, db, storage) como parâmetros.

// A função principal que "envolve" toda a lógica da página.
// Ela só é chamada depois que o Firebase está 100% pronto.
export function inicializarPerfil({ auth, db, storage }) {

    // As funções específicas de cada serviço (como onAuthStateChanged, doc, getDoc)
    // serão importadas dinamicamente ou referenciadas a partir dos objetos recebidos.
    const { onAuthStateChanged, signOut } = auth;
    const { getFirestore, doc, getDoc, setDoc, addDoc, collection, query, where, getDocs, onSnapshot } = db;
    ...
    window.addEventListener('DOMContentLoaded', () => {
        ...
        // Não precisamos mais disso, pois já recebemos auth, db, e storage.
        ...
        onAuthStateChanged(auth, (user) => {
            if (user) {
                ...
                // 1. Garante que a estrutura HTML dos dias seja criada primeiro.
                gerarEstruturaDosDias();
                
                // 2. Busca os dados do Firebase e preenche o formulário (incluindo horários e vitrine).
                verificarEcarregarDados(user.uid);
                
                // 3. Adiciona todos os listeners de eventos aos elementos.
        ...
        function preencherFormulario(dadosEmpresa, dadosProfissional) {
            if (elements.h1Titulo) elements.h1Titulo.textContent = "Meu Perfil e Configurações";
            ...
            diasDaSemana.forEach(dia => {
                ...
                if (toggleAtivo) {
                    ...
                    if (containerBlocos) {
                        containerBlocos.innerHTML = ''; // Limpa para garantir que não haja duplicatas
                        if (diaData?.ativo && diaData.blocos?.length > 0) {
                            ...
                            adicionarBlocoDeHorario(dia.id); // Adiciona um bloco padrão se estiver ativo mas sem blocos
                        }
                    }
                    // Dispara o evento 'change' para garantir que a UI (Aberto/Fechado e visibilidade) seja atualizada
